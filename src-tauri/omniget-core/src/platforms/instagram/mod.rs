...
impl PlatformDownloader for InstagramDownloader {
    ...
    fn can_handle(&self, url: &str) -> bool {
        if let Ok(parsed) = url::Url::parse(url) {
            if let Some(host) = parsed.host_str() {
                ...
                return host == "instagram.com" || host.ends_with(".instagram.com");
            }
        }
        false
    }
    ...
    async fn get_media_info(&self, url: &str) -> anyhow::Result<MediaInfo> {
        self.get_media_info_impl(url).await
    }
    ...
    async fn download(
        ...
        if count == 0 {
            return Err(anyhow::anyhow!("No media found to download"));
        }

        if count == 1 {
            let quality = info.available_qualities.first().unwrap();
            let mut rng = rand::thread_rng();
            let val: u32 = rng.gen_range(1000..10000);
            let filename = format!(
                "{}_{}.{}",
            ...
            let headers = self.get_auth_headers().await.unwrap_or_default();
        ...
        for (i, quality) in info.available_qualities.iter().enumerate() {
            ...
            let bytes =
                direct_downloader::download_direct(&self.client, &quality.url, &output, tx, Some(headers))
                    .await?;
